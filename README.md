...
1. 打开默认的「爆款采集」页面，填写账号、密码、关键词、国家/地区、三级类目、商品链接数量、每商品视频数量。
2. 点击「保存参数」。
3. 点击「一键采集」。
...
产品信息只保存在本地 `fastmoss_config.json` 的 `product_profile` 字段，不会提交到 Git。当前字段包括产品名称、产品类目、目标人群、核心痛点、核心卖点、差异化优势、使用场景、价格与优惠、信任背书、内容语气、禁用表达和补充备注。
